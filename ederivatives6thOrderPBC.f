...
c
      subroutine ederivativesPBC(f,fd,i,j,k,dfdx,dfdy,dfdz,
     1            d2fdx2,d2fdy2,d2fdz2,dfddx,dfddy,dfddz,lxb,lyl,lzd)
      implicit none
...
c central derivatives in lattice interior:
     ...
     2   -f(n,i-three2,j,k))/(60.*dx)
        dfddx(n)=(fd(n,i+three1,j,k)-9.*fd(n,i+two1,j,k)+45.
     1   *fd(n,i+one1,j,k)-45.*fd(n,i-one2,j,k)+9.*fd(n,i-two2,j,k)
     2   -fd(n,i-three2,j,k))/(60.*dx)
...
c central derivatives in lattice interior:
     ...
     2   -f(n,i,j-three2,k))/(60.*dx)
        dfddy(n)=(fd(n,i,j+three1,k)-9.*fd(n,i,j+two1,k)+45.
     1   *fd(n,i,j+one1,k)-45.*fd(n,i,j-one2,k)+9.*fd(n,i,j-two2,k)
     2   -fd(n,i,j-three2,k))/(60.*dx)
...
c central derivatives in lattice interior:
     ...
     2   -f(n,i,j,k-three2))/(60.*dx)
        dfddz(n)=(fd(n,i,j,k+three1)-9.*fd(n,i,j,k+two1)+45.
     1   *fd(n,i,j,k+one1)-45.*fd(n,i,j,k-one2)+9.*fd(n,i,j,k-two2)
     2   -fd(n,i,j,k-three2))/(60.*dx)
