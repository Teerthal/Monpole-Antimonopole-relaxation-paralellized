!---Unsure about the phased out statments but the only executed statements 
!---impose periodic boundary conditions



        subroutine ghostcellsx(f,
...
!-------------periodic boundary conditions----------------------
        ...
        if(proci==0) then
                call mpi_send(f(1:nf,bxsl:bxsu,ly:ry,dz:uz),
     1                  ghostsize,mpi_double_precision,pidfbound,
     2                  msgf,mpi_comm_world,ierr)
                call mpi_recv(f(1:nf,bxrl:bxru,ly:ry,dz:uz),
     1                  ghostsize,mpi_double_precision,pidfbound,
     2                  msgb,mpi_comm_world,status,ierr)
        endif
        if(proci==nprocessx-1) then
                call mpi_recv(f(1:nf,fxrl:fxru,ly:ry,dz:uz),
     1                  ghostsize,mpi_double_precision,pidbbound,
     2                  msgf,mpi_comm_world,status,ierr)
                call mpi_send(f(1:nf,fxsl:fxsu,ly:ry,dz:uz),
     1                  ghostsize,mpi_double_precision,pidbbound,
     2                  msgb,mpi_comm_world,ierr)
        endif
!---------------------------------------------------------------
